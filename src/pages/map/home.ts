...
export class HomePage {
  ...
  constructor(public navCtrl: NavController, private geoLocation: Geolocation, private modalCtrl: ModalController,
    ...
    this.sub = this.categorieProvider.findAllAnnonces().map(Annonces => {
  ...
  loadMap() {
    ...
    this.geoLocation.getCurrentPosition().then((resp) => {
      ...
      map.moveCamera(position).then(() => {
        let item: Annonce = { imageURL: "assets/Img/placeholder.png", titre: "Ma Position", location: { lat: userPosition.lat, lng: userPosition.lng } }
